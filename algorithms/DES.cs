...
namespace Encryption.algorithms
{
    class DES : Algorithm
    {
        public override string Encrypt(string textToEncrypt, object[] keys)
        {
            ...
            try
            {
                ulong.TryParse((string)keys[0], out key);
                ulong.TryParse(textToEncrypt, out msg);
            }
            catch (Exception ex)
            {
                return string.Empty;
            ...
            return string.Empty +  DES_Encode(msg, key);
        }
        ...
        private static readonly int[] IPINV =
        {
        ...
        private static ulong DES_Permute_PC1(ulong key)
        {
            return Permute(key, PC1);
        }

        private static ulong DES_Permute_PC2(ulong key)
        {
            return Permute(key, PC2);
        }

        private static ulong Permute(ulong val, int[] changes)
        {
            ulong result = 0;
        ...
        private static ulong DES_Key_CreateSubKeys(ulong message, ulong key)
        {
            ...
            return (pair.Right | (pair.Left >> 32));
        }

        private static ulong DES_Encode(ulong message, ulong key)
        {
            ulong joined = DES_Key_CreateSubKeys(message, key);
            return Permute(joined, IPINV);
        }


        // left half of the 56 bit 
        private static ulong Left28(ulong val)
        {
            return val & 0xFFFFFFF000000000;
        }

        // right half of the 56 bit value
        private static ulong Right28(ulong val)
        {
            return (val << 28) & 0xFFFFFFF000000000;
        }

        // concat two 56 bit 
        private static ulong Concat56(ulong left, ulong right)
        {
            return (left & 0xFFFFFFF000000000) | ((right & 0xFFFFFFF000000000) >> 28);
        }

        // 56 bit left shift
        private static ulong LeftShift56(ulong val, int count)
        {
        ...
        /**
         * 48 bits split in groups of 6 bits
         */
