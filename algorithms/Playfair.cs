...
namespace Encryption.algorithms
{
    class Playfair : Algorithm
    {
        public override string Encrypt(string textToEncrypt, object[] keys)
        {
            ...
            return DoOperation(textToEncrypt, key, true);
        }
        ...
        public override string Decrypt(string textToDecrypt, object[] keys)
        {
            ...
            return DoOperation(textToDecrypt, key, false);
        }

        private static string DoOperation(string input, string key, bool opEncipher)
        {
            string retVal = string.Empty;
            char[,] keySquare = CreateKeyMatrix(key);
            string tempInput = RemoveOtherChars(input);
            int e = opEncipher ? 1 : -1;
            ...
            retVal = PrepareOutput(input, retVal);
            return retVal;
        }
        ...
        private static char[,] CreateKeyMatrix(string key)
        {
        ...
        private static string PrepareOutput(string input, string output)
        {
